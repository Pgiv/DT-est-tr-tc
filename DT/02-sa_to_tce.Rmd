...
Nous nous intéresserons uniquement aux estimations intermédiaires de la tendance-cycle faites en temps réels.   <!-- ce paragraphe est un peu mystérieux : on s'y intéresse pourquoi? je suppose que c'est dans les exemples ? mais cela n'est pas très clair - plus généralement, je pense qu'il serait plus simple de présenter les illustrations sur données réelles au fur et à mesure -->
Le filtre symétrique utilisé pour les estimations finales ne sera pas remis en cause et sera celui d'Henderson.   <!-- je ne suis pas sure de comprendre cette phrase - quelles estimations finales? -->
Nous nous concentrons également sur les méthodes qui pourraient être implémentées dans X-13ARIMA et remplacer la méthode actuellement utilisée (prolongement de la série par modèle ARIMA puis utilisation d'un filtre symétrique).
...
Même si les différentes méthodes sont comparées en les appliquant sur des séries déjà désaisonnalisées, elles peuvent être intégrées dans l'algorithme de désaisonnalisation X-11 grâce à la fonction `rjd3filters::x11()`.
