...
Les différentes méthodes de construction de moyennes mobiles asymétriques sont comparées sur des données simulées et des données réelles. <!-- je trouve que les deux n'ont pas le même statut. Les données simulées sont notamment bcp plus proches des processus sous-jacents des méthodes, avec des belles sinusoides. C'est plus intéressant de savoir ce que cela donne sur des séries réelles. On pourrait mettre les résutlats sur données simulées en annexe, mettre des illustrations sur données réelles au fur et à mesure et récapituler dans cette seection desl performances des unes et des autres-->
-->
...
<!-- ici, je trouve que cela vaudrait la peine de préciser quels sont les dimensions sur lesquelles on compare les résutlats --> 
<!-- dans les graphiques et tableaux, il faudrait être plus explicite sur les légendes, notamment détailler les acronymes, sinon elles sont illisibles-->
...
C'est le cas à la date $t$ lorsque $y_{t-3}\leq y_{t-2}\leq y_{t-1}>y_t\geq y_{t+1}$.  
    ...
    <!-- Rep AQLT : même si je vois ton idée (on ne connait pas en avance) je trouve ce critère plus pertinent pour le choix du filtre en temps réel car on peut penser qu'une méthode qui minimise ce critère modifié donnera des points de retournement (en moyenne) plus fiables. Si l'on voit le problème autrement, lorsqu'on connait le point de retournement du fait d'une information externe, cela permet de rejeter les filtres qui donnent le bon point de retournement vite (cohérent avec l'info extérieure) mais qui conduisent à des révisions dans les estimations suivantes (pb de communication) -->
...
Par ailleurs, c'est le filtre $b_{q,\varphi}$ qui conduit à le plus de révision entre l'avant-dernière et la dernière estimation, ce qui s'explique par la différence importante entre la courbe des coefficients de la dernière moyenne mobiles asymétrique et celle du filtre symétrique.
...
Ce point et cette série ont été choisis car le retournement conjoncturel est particulièrement visible sur la série brute (voir figures \@ref(fig:ce16ov-previmp-lp) et \@ref(fig:ce16ov-previmp-autres)), que cette série est utilisée pour la datation des cycles conjoncturels de l'économie et que la base FRED-MD facilite la reproductibilité des résultats (grâce à la disponibilité des séries publiées aux dates passées).
...
Ainsi, l'estimation en temps réel des méthodes utilisant les filtres asymétriques directs (DAF) peut être facilement améliorée en utilisant le filtre LC ou QL. 
...
Pour le filtre FST, les poids retenus pour les simulations ($\alpha = 0,00$ *fidelity*, $\beta  =0,05$ *smoothness* et $\gamma = 0,95$ *timeliness*) semblent donner de bons résultats en termes de déphasage et de révisions (pour $q\geq1$).
...
En effet, plus $q$ augmente (i.e. : plus on utilise de points dans le futur), plus le déphasage et donc le critère associé à la *timeliness* est petit : le poids associé à la *smoothness* augmente donc et les moyennes mobiles trouvées se rapproche du filtre symétrique d'Henderson (qui peut être construit en ne minimisant que la *smoothness*).
