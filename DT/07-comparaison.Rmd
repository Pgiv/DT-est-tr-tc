...
C'est le cas à la date $t$ lorsque $y_{t-3}\leq y_{t-2}\leq y_{t-1}>y_t\geq y_{t+1}$.  
    ...
    <!-- Rep AQLT : même si je vois ton idée (on ne connait pas en avance) je trouve ce critère plus pertinent pour le choix du filtre en temps réel car on peut penser qu'une méthode qui minimise ce critère modifié donnera des points de retournement (en moyenne) plus fiables. Si l'on voit le problème autrement, lorsque l'on connait le point de retournement du fait d'une information externe, cela permet de rejeter les filtres qui donnent le bon point de retournement vite (cohérent avec l'info extérieure) mais qui conduisent à des révisions dans les estimations suivantes (pb de communication) -->
...
En excluant pour l'instant les paramétrisations locales des filtres polynomiaux, c'est le filtre FST qui semble donner les meilleurs résultats en termes de déphasage dans la détection des points de retournement (figure \@ref(fig:graphstpsimul)), suivi du filtre LC. 
...
Cet écart provient du fait que la fenêtre optimale retenue $b_{q,\varphi}$ est croissante jusqu'à $b_{5,\varphi}=10,39$ et s'écarte donc de la valeur utilisée du filtre symétrique est ($h+1=7$).
...
<!-- Rep AQLT : c'est effectivement un peu plus haut, je vais repréciser. -->
Par ailleurs, c'est le filtre $b_{q,\varphi}$ qui conduit à le plus de révision entre l'avant-dernière et la dernière estimation, qui s'explique par la différence importante entre la courbe des coefficients de la dernière moyenne mobiles asymétrique et celle du filtre symétrique.
...
<!-- Rep AQLT : sauf pour q=4, ajout d'une mot sur ce sujet  -->
Le prolongement de la série par un modèle ARIMA donne des révisions avec les dernières estimations du même ordre de grandeur que le filtre LC mais des révisions légèrement plus importantes entre les estimations consécutive, notamment entre la quatrième et la cinquième estimation (on pouvait s'y attendre comme souligné dans la section \@ref(subec:mmetprev)).

<!-- 2 points qui semblent très élevés, dans le tableau du bas, pour ARIMA q = 4 et b_{q,Tau} q=4 -->
<!-- Rep AQLT : Pour ARIMA pas de raison que ce soit faible (comme indiqué dans le paragraphe précédent), pour les autres je ne trouve pas d'explication mais je vais ajouter une phrase dessus -->
...
<!-- Rep AQLT : C'est bien l'emploi, j'avais changé l'exemple car le précédent était pendant le COVID et j'ai oublié de mettre à jour le titre... -->
La série étudiée est le niveau d'emploi aux États-Unis (série `CE160V`, utilisée en logarithme) autour du point de retournement de février 2001, cohérent avec la datation mensuelle des points de retournement.
Ce point et cette série ont été choisis car le retournement conjoncturel est particulièrement visible sur la série brute (voir figures \@ref(fig:ce16ov-previmp-lp) et \@ref(fig:ce16ov-previmp-autres)), que cette série est utilisée pour la datation des cycles conjoncturelle de l'économie et que la base FRED-MD facilite la reproductibilité des résultats (grâce à la disponibilité des séries publiées aux dates passées).
...
<!-- Rep AQLT : Il faut effectivement au moins 2 mois (rajouté dans la partie méthodo). Dans le sous-titre j'ai également ajouté la première détection du point de retournement. -->
<!-- On dirait effectivement qu'il est détecté rapidement pour le LC mais il n'est en fait pas détecté à la bonne date (il est détecté en janvier). -->
...
Sauf pour la méthode LC (où le point de retournement est d'abord détecté en janvier 2001), le déphasage élevé provient de révisions dans le point de retournement détecté dans les révisions intermédiaires (même si le bon point de retournement est détecté au bout de 2 mois, il ne l'est plus au bout de 3 mois).
...
<!-- Rep AQLT : j'avais inversé quelques chiffres j'ai l'impression mais oui je trouve que c'est globalement assez positif -->

La paramétrisation locale ne permet pas ici de réduire le déphasage mais a permet de réduire les révisions.
...
```{r ce16ovlp, echo=FALSE, out.width="95%", fig.cap="Estimations successives de la tendance-cycle de l'emploi (en logarithme) aux États-Unis avec les méthodes polynomiales locales."}
series <- "CE16OV"
date <- "fev2001"
...
```{r ce16ovautres, echo=FALSE, out.width="95%", fig.cap="Estimations successives de la tendance-cycle de l'emploi (en logarithme) aux États-Unis avec les RKHS, la méthode FST et en prolongeant la série par modèle ARIMA."}
img <- sprintf("img/nber/%s_%s_%s.%s",
               tolower(series),
               date,
               "autres",
...
<!-- Rep AQLT : ajout d'un paragraphe "discussion" -->
```{r ce16ov-previmp-lp, echo=FALSE, out.width="95%", fig.cap="Prévisions implicites liées aux estimations successives de la tendance-cycle de l'emploi (en logarithme) aux États-Unis avec les méthodes polynomiales locales."}
series <- "CE16OV"
date <- "fev2001"
...
```{r ce16ov-previmp-autres, echo=FALSE, out.width="95%", fig.cap="Prévisions implicites liées aux estimations successives de la tendance-cycle de l'emploi (en logarithme) aux États-Unis avec les RKHS, la méthode FST et en prolongeant la série par modèle ARIMA."}
img <- sprintf("img/nber/%s_%s_prev_imp_%s.%s",
               tolower(series),
               date,
               "autres",
...
Bien que la méthodologie employée soit perfectible, plusieurs enseignements peuvent être tirés de cette analyse empirique.

Sur les méthodes polynomiales locales, chercher à préserver des tendances polynomiales de degré supérieur à 2 (méthodes CQ et DAF) augmente la variance des estimations (et donc les révisions) et le délai pour détecter des points de retournement.
Ainsi, l'estimation en temps réel des méthodes utilisant les filtres asymétriques direct (DAF) peut être facilement améliorée en utilisant le filtre LC ou QL. 
...
La méthode proposée dans cette article pour l'estimation en temps-réel des paramètres locaux, bien que naïve et grandement perfectible, permet tout de même d'améliorer les estimations.

Concernant les méthodes basées sur les RKHS, les deux premières estimations des filtres $b_{q,\Gamma}$ et $b_{q,G}$ semblent peu cohérentes (ce qui a peu d'impact sur le déphasage puisqu'il faut au moins 2 mois pour détecter un point de retournement), comme les dernières estimations du filtre $b_{q,\varphi}$ (ce qui peut provenir d'un problème d'optimisation).
Cela suggère que la méthode utilisée pour calibrer ces filtres n'est pas optimale pour tous les horizons de prévision.
En effet, pour les premières estimations, on cherche plutôt à minimiser le déphasage ($b_{q,\varphi}$) et lorsque que l'on se rapproche du cas symétrique ($q \geq 4$) on cherche plutôt à minimiser les révisions ($b_{q,\Gamma}$ ou $b_{q,G}$).
Les performances du filtre $b_{q,\varphi}$ pourraient ainsi sûrement être améliorées en utilisant d'autres moyennes mobiles lorsque l'on l'on se rapproche du filtre symétrique ($q \geq 4$).

Pour le filtre FST, les poids retenus pour les simulations ($\alpha = 0,00$ *fidelity*, $\beta  =0,05$ *smoothness* et $\gamma = 0,95$ *timeliness*) semblent donner de bon résultats en termes de déphasage et de révisions (pour $q\geq1$).
Les critères utilisés dans cette méthode n'étant pas normalisés, les poids retenus reviennent à donner un poids plus important à la *timeliness* pour les premières estimations et un poids plus important à la *smoothness* lorsque l'on se rapproche du cas d'utilisation du filtre symétrique.
En effet, plus $q$ augmente (i.e. : plus on utilise de points dans le futur), plus le déphasage et donc le critère associé à la *timeliness* est petit : le poids associé à la *smoothness* augmente donc et les moyennes mobiles trouvées se rapproche du filtre symétrique d'Henderson (qui peut être construit en minimisant que la *smoothness*).
...
L'application directe des méthodes peut donc conduire donc à des estimations biaisées, du fait de leur présence, alors que les méthodes de désaisonnalisation (comme la méthode X-13ARIMA) ont un module de correction des points atypiques.
Par ailleurs, comme notamment montré par @dagum1996new, le filtre symétrique final utilisé par X-13ARIMA pour extraire la tendance-cycle (et donc celui indirectement utilisé lorsque l'on applique les méthodes sur les séries désaisonnalisées) laisse passer environ 72 % des cycles de 9 ou 10 mois (généralement associés à du bruit plutôt qu'à la tendance-cycle.
