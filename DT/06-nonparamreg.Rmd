...
Le noyau trapézoïdal est implémenté dans `rjd3filters` car il permet de calculer les moyennes mobiles utilisées dans l'algorithme X-13ARIMA pour l'extraction des composantes saisonnières. 
...
Ces filtres asymétriques dépendent du rapport $\lvert\delta_1/\sigma\rvert$, qui est lié à l'I-C ratio $R=\frac{\bar{I}}{\bar{C}}=\frac{\sum\lvert I_t-I_{t-1}\rvert}{\sum\lvert C_t-C_{t-1}\rvert}$ ($\delta_1/\sigma=2/(R\sqrt{\pi})$), qui est notamment utilisé dans X-13ARIMA pour déterminer la longueur du filtre d'Henderson^[
...
Pour paramétrer les filtres asymétriques, on pourrait préférer une paramétrisation locale (rapport $\lvert\delta/\sigma\rvert$ qui varie en fonction du temps) à une paramétrisation globale ($\lvert\delta/\sigma\rvert$ estimé à partir l'ensemble des données à partir de l'IC-ratio ou d'un critère de validation croisée).
...
Par exemple, pour la méthode LC on peut utiliser la moyenne mobile $\theta_2=KX(X'KX)^{-1}e_{2}$ pour avoir un estimation locale de la pente et pour la méthode QL on peut utiliser $\theta_3=KX(X'KX)^{-1}e_{3}$ pour avoir une estimation locale de la concavité. 
La méthode DAF permet alors de simplement calculer les moyennes mobiles asymétriques associées.  
Une moyenne mobile de longueur différente de celle utilisée pour l'estimation de la tendance pourrait être envisagée. 
En utilisant la même méthodologie que dans la section \@ref(sec:sec-comparison), cela semble dégrader les résultats.   
Par ailleurs, pour la construction des moyennes mobiles, utiliser la méthode DAF en modélisant une tendance de degré 2 semble donner des meilleurs résultats qu'en utilisant une tendance de degré 3. 
La figure \@ref(fig:mmpenteconcac) montre les moyennes mobiles utilisées.

```{r mmpenteconcac, echo=FALSE, out.width="90%", fig.cap="Moyennes mobiles utilisées pour l'estimations de la pente et de la concavité."}
img <- sprintf("img/filters_used/mm_penteconcavite.%s", fig.ext)
knitr::include_graphics(img)
```

Dans les applications empirique, la paramétrisation locale finale correspond à celle où $delta$ est estimé en utilisant l'ensemble des données (c'est-à-dire en utilisant les filtres symétriques de la figure \@ref(fig:mmpenteconcac)) mais en gardant une estimation en temps réel de $\sigma^2$.
...
Il est en revanche possible de généraliser davantage la modélisation en ajoutant le critère de *timeliness* définit par @ch15HBSA dans l'équation \@ref(eq:lppasym). 
...
Ainsi, la valeur optimal retenue par défaut par `rjd3filters` produit des discontinuités dans l'estimation de la tendance-cycle.  
