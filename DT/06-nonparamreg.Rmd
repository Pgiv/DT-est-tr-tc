...
<!-- c'est tout à fait vrai, mais on se demande un peu quel est le point ici, surtout après la présentation des deux filtres précédents. Je propose d'inverser l'ordre des paragraphe pour être au clair-->


De nombreuses méthodes d'extraction de la tendance-cycle sont fondées sur des régressions non-paramétriques, particulièrement souples car elles ne supposent pas de dépendance prédéterminée dans les prédicteurs. En pratique, on peut s'appuyer sur des régressions locales (comme montré par exemple par @Loader1999). <!-- dans la mesure où c'est une application directe de Taylor, est-ce c'est vraiment le premier à avoir démontré ça et si tardivement?? mais peut être en effet -->  Plus précisément, si on considère un ensemble de points $(x_i,y_i)_{1\leq i\leq n}$. 
...
Concernant le choix des paramètres, l'idée générale qui prévaut est que le choix entre ces différents noyaux est secondaire <!-- oui c'est assez général et c'est pour cela que je préconiserai de mettre la section suivante en annexe-->^[
...
### Les différents noyaux {#sec-kernels}<!-- très classique et pas central ici, comme d'ailleurs dit plus haut : à mettre en annexe.  --> 
...
### Quelques filtres symétriques particuliers {#sec-sympolyfilter}<!-- pas sure que ce soit indispensable. PAr ailleurs, dans la première partie il est indiqué qu'on se concentre sur le filtre symétrique d'Henderson). Commencer par ça? également renvoyer en annexe? -->

Lorsque fenêtre de la moyenne mobile <!-- pour préciser, c'est bien ça - le risque étant de mélanger avec la fenêtre du kernel... mais sinon ce p tombe un peu comme un cheveu sur la soupe. Ou alors, rappeler qqch comme : pour une moyenne mobile ...--> $p=0$ (ajustement local par une constante) on obtient l'estimateur de **Nadaraya-Watson** (ou l'estimateur par noyaux).
...
Comme mentionné dans la partie \@ref(subec:mmetprev), pour l'estimation en temps réel, plusieurs approches peuvent être utilisées :<!-- il ne me semble pas que ce soit dit aussi directement dan la partie  \@ref(subec:mmetprev) (et ça mériterait de l'être)... quoique là non plus je ne suis pas certaine de voir complétement ce qui décrit. Encore une fois, une illustration concrète commetnée permettrait peut être de mieux comprendre-->
...
Dans la majorité des cas un filtre de 13 termes <!-- Question naive : pourquoi 13? 12+1? mais du coup pourquoi 23?--> est utilisé.
...
Le tableau \@ref(tab:criteriaLp) compare les critères de qualité des différentes méthodes en utilisant le filtre d'Henderson et $h=6$ (filtre symétrique de 13 termes). <!-- là je suis perdue : ils sont calculés sur quoi? CE serait bien par ailleurs d'être plus explicite dans la table (aussi vrai plus bas sur les applications, mettre légende avec le sens des abbréviations -->
...
Dans cet article, nous proposons d'effectuer une paramétrisation locale des filtres asymétriques en estimant séparément $\delta$ et $\sigma^2$ :
Même si cela ne donne pas un estimateur sans biais du rapport $\lvert\delta/\sigma\rvert$, cela permet de capter les principales évolutions comme la décroissance vers 0 avant un point de retournement et la croissance après le point de retournement pour la méthode LC <!-- pour éviter de se retrouver avec un sigma puissance 222 mais aussi car peut aider à comprendre le choix-->:
...
<!-- ref de jean palate? Rep AQLT : pas de référence, il l'a juste implémenté : du coup je trouve bizarre de le mettre dans le texte, peut être footnote?-->
...
$y_t=TC_t+I_t$.<!-- ce qui est paradoxal c'est que je suppose que c'est qqch qu'on peut dire aussi plus haut, non? Du coup cela fait bizarre que cela n'arrive qu'à la page 30 -->
...
:::: {.remarque data-latex=""}<!-- inutile dans le texte, à mettre en footnote-->
...
## Liens entre les différentes méthodes {#subsec-equivlpfst}<!-- je pense qu'on pourrait simplement le mettre en annexe, cela n'apporte pas grand chose même si intellectuellement c'est intéressant-->
