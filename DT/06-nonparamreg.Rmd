...
<!-- a moins qu’on ne désigne pas le noyau K par sa densité kappa? -->
...
<!-- pas clair pour moi: c’est les constantes qui pourraient changer? le 1/3, 2/3 et 3/3 pour trapézoïdal? -->
...
<!-- sépare les 2 formules s’il ne faut pas penser que la parenthèse $\delta_1/\sigma=2/(R\sqrt{\pi})$ multiplie le ratio -->
...
<!-- si $A_w+S_w+T_w+R_w$ est une grandeur pertinente, ajouter une colonne avec cette somme ? je comprends aussi qu’on ne sait pas bien agréger les 3 F, S et T, donc difficile de dire a priori lequel on préfèrerait.
peut-être ajouter un peu plus d’analyse économique de ces résultats ou de conséquence sur la pratique de l’économètre ? -->
...
Il est caractérisé par une fonction de densité $f_0$ et un produit scalaire $\ps{\cdot}{\cdot}$ défini par :
$$
...
On peut, par ailleurs, montrer que $K_{d+1}$ s'écrit en fonction de $f_0$ et du noyau reproduisant $K(\cdot,\cdot)$ et que ce dernier peut s'écrire en fonction de polynômes $(P_i)_{i\in \llbracket 0, d-1 \rrbracket}$ qui forme une base orthonormée de $\mathbb L^2(f_0)$ (voir par exemple @berlinet1993) :
$$
...
$$
<!-- est-ce qu’il faut définir diagramme des coefficients? -->
