...
Toutes ces méthodes peuvent se voir comme des cas particulier d'une formulation générale de construction des moyennes mobiles, ce qui permet de montrer leurs similitudes et leurs différences et ainsi les comparer plus facilement.
...
Deuxièmement, en fin de période, modéliser des tendances polynomiales de degré supérieur à trois (CQ et DAF) semble introduire de la variance dans les estimations (et donc plus de révisions) sans permettre de détection plus rapide des points de retournement.
En fin de période, pour l'estimation de la tendance-cycle, nous pouvons donc nous restreindre aux méthodes modélisant des tendances polynomiales de degré au plus 2 (LC et QL).
...
Même lorsque le déphasage n'est pas diminué, la paramétrisation locale est recommandée car elle permet de réduire les révisions et d'avoir des estimations intermédiaire plus cohérentes avec les évolutions futures attendues.
En revanche, avec ces méthodes, la longueur du filtre utilisé doit être adapté à la variabilité de la série : si le filtre utilisé est trop court (c'est-à-dire si la variabilité de la série est « faible »), conserver des tendances polynomiale de degré au plus 1 (méthode LC) produit de moins bons résultats en termes de détection des points de retournement.

Enfin, sur les méthodes basées sur l'optimisation d'une somme pondérée de certains critères (FST, RKHS, ATS), les performances pourraient être améliorées en utilisant des critères différents pour les différents filtres asymétriques.
En effet, pour les premières estimations (lorsque peu de points dans le futur sont connus), le conjoncturiste peut préférer minimiser le déphasage alors que lorsque l'on se rapproche du cas d'utilisation du filtre symétrique (c'est-à-dire lorsque le se rapproche des estimations finales), le critère le plus important pourrait être la réduction des révisions.
...
Pour le filtre $b_{q,\varphi}$, cela conduit par exemple à de grandes révisions entre l'avant-dernière et la dernière estimation.
