...
@ch15HBSA proposent une approche générale pour dériver des filtres linéaires, fondée sur un problème d'optimisation de trois critères : *Fidelity* ($F_g$, réduction de la variance), *Smoothness* ($S_g$, lissage) et *Timeliness* ($T_g$, déphasage).
Voir section \@ref(subsec-GuggemosEtAl) pour plus de détails.

@trilemmaWMR2019 proposent une approche qui s’appuie sur la décomposition de l'erreur quadratique moyenne entre le filtre symétrique et le filtre asymétrique en quatre quantités : *Accuracy* ($A_w$, précision), *Timeliness* ($T_w$, déphasage), *Smoothness* ($S_w$, lissage) et *Residual* ($R_w$, résidus).
